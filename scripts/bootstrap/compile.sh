...
# Brace expansion is a bash-ism; POSIX sh leaves "a/{b,c}" as written, so the
# files are listed out in full.
_OPTIONS_DIR=src/main/java/com/google/devtools/common/options
ANNOTATION_PROCESSOR_FILES="${_OPTIONS_DIR}/processor/OptionsClassProcessor.java src/main/java/com/google/devtools/build/lib/skybridge/SkybridgeInterface.java ${_OPTIONS_DIR}/Option.java ${_OPTIONS_DIR}/OptionMetadataTag.java ${_OPTIONS_DIR}/OptionEffectTag.java ${_OPTIONS_DIR}/OptionDocumentationCategory.java ${_OPTIONS_DIR}/OptionsClass.java ${_OPTIONS_DIR}/Converter.java ${_OPTIONS_DIR}/OptionsParsingException.java ${_OPTIONS_DIR}/processor/OptionProcessorException.java ${_OPTIONS_DIR}/processor/ProcessorUtils.java"
...
java_compilation() {
  ...
  # POSIX sh has no ${var//pat/rep}; tr turns the space-separated lists into
  # path lists the same way.
...
build_jni() {
  ...
  local tmp_output="${NEW_TMPDIR}/jni/$(basename "${output}")"
...
run_bazel_jar() {
  ...
  # There are no arrays in POSIX sh, so the --client_env flags are appended to
  # the positional parameters; their order relative to the command's own
  # arguments does not matter to Bazel.
