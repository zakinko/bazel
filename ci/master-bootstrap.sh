...
# compile.sh の第二段は //:bootstrap-derived-java-jars_checked_in を要求する。
# これは derived/jars/**/*.jar を glob する filegroup で、dist archive にしか
# 中身が無い。空だと
#
#	in jars attribute of bootstrap_java_library rule ...:starlark-deps:
#	  '//:bootstrap-derived-java-jars_checked_in' does not produce any
#	  bootstrap_java_library jars files (expected .jar)
#
# になる。maven から取った jar をそちらにも置く。
echo "=== derived/jars を埋める"
mkdir -p "$SRC/derived/jars"
find "$SRC/derived/maven" -name '*.jar' | while read j; do
	ln -f "$j" "$SRC/derived/jars/$(basename "$j")" 2>/dev/null ||
		cp "$j" "$SRC/derived/jars/$(basename "$j")"
done
echo "  $(ls "$SRC/derived/jars" | wc -l | tr -d ' ') 個"
